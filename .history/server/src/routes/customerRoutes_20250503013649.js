...
customerRoute.delete('/cancel-ticket', async (req, res) => {
  ...
  try {
    ...
    console.log('Ownership check query:', 
      `SELECT P.Ticket_ID FROM Purchase P JOIN Ticket T ON P.Ticket_ID = T.Ticket_ID WHERE P.Ticket_ID = ${ticketId} AND P.Customer_Email = '${customerEmail}'`);
    ...
    console.log('Ownership check results:', ownershipCheck[0]);
    
    if (!ownershipCheck[0] || ownershipCheck[0].length === 0) {
      console.log('Ownership verification failed for ticket:', ticketId);
      return res.status(403).json({ error: 'Ticket not found or unauthorized' });
    }
    
    // In the flight data section:
