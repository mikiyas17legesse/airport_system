...
authRoute.post('/staff-signup', (req, res) => {
    ...
    connection.query('SELECT * FROM Airline_Staff WHERE Username = ?', [username], (err, results) => {
        ...
        connection.query('SELECT * FROM Airline WHERE Name = ?', [airlineName], (err, results) => {
            if (err) return res.status(500).json({ message: 'Database error.' });
            if (results.length === 0) {
        ...
        connection.query(
            ...
            (err, results) => {
                ...
                console.log('Passed Airline_Staff');
                ...
                connection.query(
                    ...
                    (err, results) => {
                        if (err) return res.status(500).json({ message: 'Database error.' });
                        console.log('Passed Airline_Staff_Email');
                        ...
                        connection.query(
                            ...
                            (err, results) => {
                                if (err) return res.status(500).json({ message: 'Database error.' });
                                console.log('Passed Airline_Staff_Phone');
