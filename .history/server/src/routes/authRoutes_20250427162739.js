...
authRoute.post('/staff-signup', (req, res) => {
    ...
    connection.query('SELECT * FROM Staff WHERE Username = ?', [username], (err, results) => {
        ...
        connection.query(
            'INSERT INTO Staff (Username, First_Name, Last_Name, Password, Email, Phone_Number, Date_Of_Birth) VALUES (?, ?, ?, ?, ?, ?, ?)',
            [username, firstName, lastName, password, email, phoneNumber, dateOfBirth],
            (err, results) => {
                if (err) return res.status(500).json({ message: 'Database error.' });
