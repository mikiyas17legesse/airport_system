...
authRoute.post('/staff-signup', (req, res) => {
    ...
    connection.query('SELECT * FROM Airline_Staff WHERE Username = ?', [username], (err, results) => {
        ...
        connection.query(
            'INSERT INTO Airline_Staff (Username, Password, First_Name, Last_Name, Phone_Number, Date_Of_Birth, Airline_Name) VALUES (?, ?, ?, ?, ?, ?, ?)',
            [username, password, firstName, lastName, phoneNumber, dateOfBirth, airlineName],
            (err, results) => {
                if (err) return res.status(500).json({ message: 'Database error.' });

                // Insert into Airline_Staff_Email
                connection.query(
                    'INSERT INTO Airline_Staff_Email (Email, Username) VALUES (?, ?)',
                    [email, username],
                    (err, results) => {
                        if (err) return res.status(500).json({ message: 'Database error.' });
                        ...
                        connection.query(
                            'INSERT INTO Airline_Staff_Phone (Phone_Number, Username) VALUES (?, ?)',
                            [phoneNumber, username],
                            (err, results) => {
                                if (err) return res.status(500).json({ message: 'Database error.' });

                                // Success! Only now do we send a response
