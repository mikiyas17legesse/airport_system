...
authRoute.post('/staff-signup', (req, res) => {
    ...
    connection.query('SELECT * FROM Airline_Staff WHERE Username = ?', [username], (err, results) => {
        ...
        connection.query(
            'INSERT INTO Airlien_Staff (Username, Password, First_Name, Last_Name, Phone_Number, Date_Of_Birth, Airline_Name) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
        ...
        connection.query(
            'INSERT INTO Airline_Staff_Email (Email, Airline_Name) VALUES (?, ?)',
            [email, airlineName],
            (err, results) => {
                if (err) return res.status(500).json({ message: 'Database error.' });
        ...
        connection.query(
            'INSERT INTO Airline_Staff_Phone (Phone_Number, Airline_Name) VALUES (?, ?)',
            [phoneNumber, airlineName],
            (err, results) => {
                if (err) return res.status(500).json({ message: 'Database error.' });
