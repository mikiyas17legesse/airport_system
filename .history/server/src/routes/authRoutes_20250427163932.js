...
authRoute.post('/staff-signup', (req, res) => {
    ...
    // Checks if all fields are provided
    if (!username || !password || !email || !phoneNumber || !dateOfBirth || !airlineName) {
        return res.status(400).json({ message: 'All fields are required.' });
    }
    // Check if user exists
    connection.query('SELECT * FROM Staff WHERE Username = ?', [username], (err, results) => {
        if (err) return res.status(500).json({ message: 'Database error.' });
        if (results.length > 0) {
        ...
        connection.query(
            'INSERT INTO Staff (Username, Password, First_Name, Last_Name, Phone_Number, Date_Of_Birth, Airline_Name) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
