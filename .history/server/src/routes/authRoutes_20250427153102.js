...
authRoute.post('/signup', async (req, res) => {
    ...
    if (!email || !firstName || !lastName || !password || !confirmPassword || !buildingNum || !street || !aptName || !city || !state || !zip || !passportNum || !passportExpiration || !passportCountry || !dateOfBirth || !phoneNumber) {
    ...
    connection.query('SELECT * FROM Customer WHERE email = ?', [email], (err, results) => {
        ...
        connection.query('INSERT INTO Customer (password, email, firstName, lastName, buildingNum, street, aptName, city, state, zip, passportNum, passportExpiration, passportCountry, dateOfBirth, phoneNumber) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [password, email, firstName, lastName, buildingNum, street, aptName, city, state, zip, passportNum, passportExpiration, passportCountry, dateOfBirth, phoneNumber], (err, results) => {
            if (err) return res.status(500).json({ message: 'Database error.' });
