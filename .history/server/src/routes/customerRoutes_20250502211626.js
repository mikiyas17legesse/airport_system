...
customerRoute.post('/rate-flight', async (req, res) => {
  console.log('Received rating request with body:', req.body);
  ...
  if (!customer_email) {
    console.log('Unauthorized request - no customer email');
    return res.status(401).json({ error: 'Unauthorized' });
  }

  try {
    console.log('Executing eligibility check query with params:', {
      customer_email, airline_name, flight_num, depart_date, depart_time
    });
    ...
    const results = await connection.query(`
      ...
      JOIN Flight f ON t.Airline_Name = f.Airline_Name AND t.Flight_Num = f.Flight_Num
                   AND t.Depart_Date = f.Depart_Date AND t.Depart_Time = f.Depart_Time
    ...
    console.log('Processed eligibility check:', eligibilityCheck);

    if (!eligibilityCheck) {
      console.log('Eligibility check failed - no results found');
    ...
    if (err.code === 'ER_DUP_ENTRY' || err.code === '23505') { // MySQL / Postgres duplicate key error
      return res.status(409).json({ error: 'You have already rated this flight.' });
    }
    console.error(err);
