...
customerRoute.delete('/cancel-ticket', async (req, res) => {
  ...
  try {
    ...
    // 1. Verify purchase - using query() instead of execute()
    const purchaseQuery = await connection.query(
      'SELECT * FROM Purchase WHERE Ticket_ID = ? AND Customer_Email = ?',
      [ticketId, customerEmail]
    );
    const purchase = purchaseQuery[0]; // First array element contains rows

    if (!purchase || purchase.length === 0) {
      console.error('Purchase verification failed:', {
        ticketId,
        customerEmail,
        purchaseResult: purchaseQuery
      });
      return res.status(403).json({ error: 'Ticket not found in your purchases' });
    }

    // 2. Verify flight information
    const flightQuery = await connection.query(
    ...
    const flightData = flightQuery[0];

    if (!flightData || flightData.length === 0) {
      console.error('Flight data not found:', { ticketId, flightResult: flightQuery });
    ...
    if (deleteResult[0].affectedRows === 0) {
      ...
      return res.status(500).json({ error: 'No records deleted' });
    }
